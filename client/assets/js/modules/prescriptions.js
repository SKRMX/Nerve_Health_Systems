...
function addRxSheet() {
  ...
  renderRxBuilder({ keepSheets: true });
...
function updatePreview() {
  ...
  const fSize = Math.max(0.45, baseFontSize * Math.pow(scale, 0.45));
...
async function renderRxHistory() {
  ...
  // Grouping logic:
  // 1. Group by batchId if present.
  // 2. For items without batchId, group by (patientId + date) if they were created very close in time.
  const groups = {};
  rxData.forEach(r => {
    let key;
    if (r.batchId) {
      ...
      // Legacy grouping: same patient, same day (YYYY-MM-DD)
      const datePart = new Date(r.createdAt).toISOString().split('T')[0];
      key = `legacy-${r.patientId}-${datePart}`;
    }
  ...
  window.reprintBatch = async (batchKey) => {
    const batch = groups[batchKey];
